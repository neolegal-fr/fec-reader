...
5. `pdf/ExtracteurQuadrillage` → seconde lecture indépendante par la grille du
   tableau (Tabula), quand le document dessine ses bordures. L'algorithme vient
   de `SpreadsheetExtractionAlgorithm.neolegalDefaults()`, du fork
   `fr.neolegal:tabula`, qui porte le réglage calibré sur ce jeu d'essai : ne pas
   le remplacer par le constructeur par défaut, la lecture tomberait de 99,6 % à
   96,6 % ;
